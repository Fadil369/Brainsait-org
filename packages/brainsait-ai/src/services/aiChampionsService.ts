...
export class AIChampionsService {
  async enrollChampion(data: z.infer<typeof ChampionEnrollmentSchema>) {
  ...
  private async createOnboardingTasks(championId: string) {
  ...
  async updateProficiency(data: z.infer<typeof ProficiencyUpdateSchema>) {
    ...
    const weightedScore = this.calculateWeightedScore(
      champion.proficiencyScore,
      validated.score,
      validated.assessmentType
    );
  ...
  private calculateWeightedScore(
    ...
    const weight = weights[assessmentType as keyof typeof weights] || 0.1;
    return currentScore * (1 - weight) + newScore * weight;
  }

  async submitUseCase(data: z.infer<typeof UseCaseSubmissionSchema>) {
  ...
  async getChampionMetrics(championId: string): Promise<ChampionMetrics> {
    const champion = await prisma.aIChampion.findUnique({
      where: { id: championId },
      include: {
        aiUsageLogs: true,
    ...
    const totalUsageHours = champion.aiUsageLogs.reduce(
      (sum, log) => sum + (log.timeSaved || 0) / 60,
      0
    );

    const tokensUsed = champion.aiUsageLogs.reduce(
      (sum, log) => sum + log.promptTokens + log.completionTokens,
      0
    );

    const timeSaved = champion.aiUsageLogs.reduce(
      (sum, log) => sum + (log.timeSaved || 0),
      0
    );
  ...
  async calculateROI(projectId: string) {
    ...
    const timeSavedHours = project.champion.aiUsageLogs.reduce(
      (sum, log) => sum + (log.timeSaved || 0) / 60,
      0
    );

    const hourlyRate = 50;
    const tokenCost = project.champion.aiUsageLogs.reduce(
      (sum, log) => sum + (log.promptTokens + log.completionTokens) * 0.00001,
      0
    );
