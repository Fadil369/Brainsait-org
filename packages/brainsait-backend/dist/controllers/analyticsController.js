...
exports.getMyAnalytics = exports.exportAnalytics = exports.getProgramAnalytics = exports.getSMEAnalytics = exports.getDashboardAnalytics = void 0;
...
exports.getDashboardAnalytics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    ...
    try {
        const [totalUsers, totalSMEs, verifiedSMEs, totalPrograms, activePrograms, totalEnrollments, activeEnrollments, completedEnrollments, recentUsers, recentSMEs, programsByType, enrollmentsByStatus, usersByRole, monthlyGrowth,] = await Promise.all([
            // Basic counts
            prisma.user.count(),
 ...
 * Get SME analytics
 */
exports.getSMEAnalytics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    if (!req.user || !['ADMIN', 'SUPER_ADMIN'].includes(req.user.role)) {
        return res.status(403).json({
            success: false,
            error: {
    ...
    try {
        const [smesByType, smesByIndustry, smesByVerificationStatus, smesByFoundedYear, smesByEmployeeCount,] = await Promise.all([
        ...
        logger_1.logger.error('Error fetching SME analytics:', error);
        res.status(500).json({
            success: false,
            error: {
 ...
 */
exports.getProgramAnalytics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    if (!req.user || !['ADMIN', 'SUPER_ADMIN'].includes(req.user.role)) {
        return res.status(403).json({
            success: false,
            error: {
    ...
    try {
        const [programStats, enrollmentTrends, completionRates, popularPrograms,] = await Promise.all([
            getProgramStats(),
            getEnrollmentTrends(),
            getCompletionRates(),
            getPopularPrograms(),
        ...
        logger_1.logger.error('Error fetching program analytics:', error);
        res.status(500).json({
            success: false,
            error: {
 ...
 */
exports.exportAnalytics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    if (!req.user || !['ADMIN', 'SUPER_ADMIN'].includes(req.user.role)) {
        return res.status(403).json({
            success: false,
            error: {
    ...
    if (!type || !['users', 'smes', 'programs', 'enrollments'].includes(type)) {
    ...
    try {
        ...
        logger_1.logger.info(`Analytics data exported: ${type} as ${format}`, {
            adminId: req.user.id,
            exportType: type,
            format,
        });
 ...
 */
exports.getMyAnalytics = (0, errorHandler_1.asyncHandler)(async (req, res) => {
    if (!req.user) {
    ...
    try {
        const [enrollmentStats, progressData, programHistory,] = await Promise.all([
...
async function getSMEsByEmployeeCount() {
    ...
    smes.forEach(sme => {
        if (sme.employeeCount !== null) {
            let sizeCategory;
            if (sme.employeeCount <= 10)
                sizeCategory = '1-10';
            else if (sme.employeeCount <= 50)
                sizeCategory = '11-50';
            else if (sme.employeeCount <= 200)
                sizeCategory = '51-200';
            else
                sizeCategory = '200+';
...
async function getMyEnrollmentStats(smeId) {
...
async function getMyProgressData(smeId) {
    return prisma.programEnrollment.findMany({
        where: { smeId },
        select: {
            progress: true,
...
async function getMyProgramHistory(smeId) {
    return prisma.programEnrollment.findMany({
        where: { smeId },
        include: {
            program: {
