...
export class SaudiComplianceService {
   ...
   */
  async createOrUpdateCompliance(smeId: string, data: SaudiComplianceData): Promise<any> {
    try {
      ...
      const complianceData = {
        ...
        waslRegion: data.waslAddress?.region as SaudiRegion,
   ...
   */
  async performComplianceCheck(smeId: string): Promise<ComplianceCheckResult> {
   ...
   */
  async validateCRWithGovernment(crNumber: string, smeId: string): Promise<boolean> {
    try {
      ...
      await this.logAPICall(
        ...
        { crNumber },
        isValid ? undefined : 'Invalid CR number format',
        'https://api.moci.gov.sa/cr/validate',
        responseTime
      );
   ...
   */
  private generateRecommendations(issues: ComplianceIssue[], compliance: any): string[] {
   ...
   */
  private calculateNextAuditDate(issueCount?: number): Date {
    const now = new Date();
   ...
   */
  async getComplianceSummary(smeId: string): Promise<any> {
    const compliance = await prisma.saudiRegulatoryCompliance.findUnique({
      where: { smeId },
      include: {
        sme: {
