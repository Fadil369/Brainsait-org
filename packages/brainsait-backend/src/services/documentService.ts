...
export class DocumentService {
  ...
  private static outputPath = path.join(process.cwd(), 'uploads', 'generated-documents');
   ...
   */
  static async generateFeasibilityStudy(data: FeasibilityStudyData): Promise<{
    filePath: string;
    fileName: string;
    downloadUrl: string;
  }> {
   ...
   */
  static async generateBusinessPlan(data: BusinessPlanData): Promise<{
    filePath: string;
    fileName: string;
    downloadUrl: string;
  }> {
   ...
   */
  private static generateFeasibilityStudyHTML(data: FeasibilityStudyData): string {
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
   ...
   */
  private static generateBusinessPlanHTML(data: BusinessPlanData, smeProfile: any): string {
